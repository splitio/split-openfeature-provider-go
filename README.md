...
[![Coverage](https://img.shields.io/badge/coverage-85.0%25-brightgreen?style=for-the-badge&logo=go)](https://github.com/splitio/split-openfeature-provider-go)
...
**OpenFeature Go Provider for [Split](https://www.harness.io/products/feature-management-experimentation) (Harness FME)**
...
OpenFeature provider for [Split](https://www.harness.io/products/feature-management-experimentation) enabling feature flag evaluation through the OpenFeature SDK with support for
...
provider, err := split.New("YOUR_SDK_KEY")
if err != nil {
	log.Fatal(err)
}
...
provider, err := split.New("YOUR_SDK_KEY", split.WithSplitConfig(cfg))
```
...
Use named providers for multi-project or service-isolated configurations:
...
project1Provider, _ := split.New("PROJECT_1_SDK_KEY")
openfeature.SetNamedProviderWithContextAndWait(ctx, "project-1", project1Provider)

project2Provider, _ := split.New("PROJECT_2_SDK_KEY")
openfeature.SetNamedProviderWithContextAndWait(ctx, "project-2", project2Provider)

// Create clients for each named provider domain
client1 := openfeature.NewClient("project-1")
client2 := openfeature.NewClient("project-2")
```
...
```go
newProvider, _ := split.New("YOUR_SDK_KEY")
```

#### Thread Safety Guarantees

The provider is thread-safe with the following guarantees:
...
client := openfeature.NewClient("my-app") // or named domain like "project-1"
...
- **[test/integration/](./test/integration/)** - Integration test suite (85 localhost / 94 cloud tests)
...
- Default values keep the application running during outages or misconfigurations
...
|----------------------------|-----------|--------------------------|
| `string`                   | Yes | `Add("currency", "USD")`   |
| `bool`                     | Yes | `Add("is_premium", true)`  |
| `int`, `int32`, `int64`    | Yes | `Add("item_count", 3)`     |
| `uint`, `uint32`, `uint64` | Yes | `Add("quantity", uint(5))` |
| `float32`, `float64`       | Yes | `Add("price", 99.99)`      |
| `nil`                      | Yes | `Add("optional", nil)`     |
| Arrays, maps, structs      | No  | Silently set to `nil`      |

**Note:** Unsupported types (arrays, maps, nested objects) are **silently set to `nil`** by the Split SDK - no
...
- Do not call `factory.Client().Destroy()` — the provider owns lifecycle
- Do not call `factory.Client().BlockUntilReady()` — use `client.State()` instead (see [Provider Status](#provider-status))
- Factory is only valid between `Init` and `Shutdown`
- After `Shutdown()`, factory and client are destroyed
...
- [Harness Feature Management & Experimentation](https://www.harness.io/products/feature-management-experimentation)
