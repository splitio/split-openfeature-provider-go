...
func (p *Provider) waitForInitGoroutines(ctx context.Context) error {
	...
	// This goroutine may outlive the function if ctx expires before initDone,
	// but it is bounded by the BlockUntilReady timeout configured on the SDK.
	go func() {
		p.initMu.Lock()
		p.initWg.Wait()
		p.initMu.Unlock()
...
func (p *Provider) Status() of.State {
	// RLock prevents factory from being nilled during shutdown (write lock in ShutdownWithContext).
	p.mtx.RLock()
...
func (p *Provider) Metrics() ProviderMetrics {
	// RLock prevents factory from being nilled during shutdown (see Status()).
	p.mtx.RLock()
	shutdown := p.shutdown.Load()
	factory := p.factory
	p.mtx.RUnlock()
