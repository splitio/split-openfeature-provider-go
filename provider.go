...
//   - Spawned: During InitWithContext after SDK is ready (lifecycle.go:155)
...
//   - Tracking: monitorDone channel closed when goroutine exits (events.go:100)
//   - Safety: Panic recovery ensures monitorDone always closed (events.go:107-111)
//
// 2. **Initialization Goroutine** (BlockUntilReady wrapper)
//   - Spawned: During InitWithContext to monitor SDK initialization (lifecycle.go:78-82)
...
//   - Cleanup: ShutdownWithContext blocks on initWg.Wait() before destroying client (lifecycle.go:289-292)
...
//
// 3. **Shutdown Goroutine** (client.Destroy wrapper)
//   - Spawned: During ShutdownWithContext to destroy Split SDK client (lifecycle.go:251-260)
...
//   - Documentation: This is a known Split SDK limitation (lifecycle.go:233-235)
