...
//   - Treatment: string (the Split treatment name, or "control" if the flag could not be evaluated)
//   - Config: any (parsed JSON config, supports objects/arrays/primitives, or nil)
//
// In flag set mode, individual flags may return Treatment "control" if the SDK
// cannot evaluate them (e.g., flag not yet cached). Callers should check for
// "control" and handle it as an unavailable flag.
//
