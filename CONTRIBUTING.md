...
```bash
task integration-test  # Localhost mode (no API key needed)
task advanced-test     # Cloud-only features (requires SPLIT_API_KEY)
```
...
**Advanced Test (`test/advanced/`)** - Cloud-only features:

- Flag set evaluation
- Complex attribute-based targeting rules
- Interactive configuration change detection
...
```bash
SPLIT_API_KEY="your-key" task integration-test
```
...
# Advanced test - cloud mode (requires API key)
SPLIT_API_KEY="your-key" task advanced-test
```

**Recommendation:** Run `task integration-test` before submitting PRs that affect:
...
task integration-test  # Integration tests (localhost)
task advanced-test     # Advanced tests (cloud, requires API key)
